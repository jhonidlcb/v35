              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground line-through">
                    ₲ {exchangeRate ? Math.round(parseFloat(latestPending.originalPrice) * parseFloat(exchangeRate.usdToGuarani)).toLocaleString('es-PY') : 'Cargando...'}
                  </p>
                </div>
                <div>
                  ...
                  {latestPending.proposedPrice && exchangeRate && (
                    <p className="text-sm text-muted-foreground mt-2">
                      Equivalente: ₲ {Math.round(parseFloat(latestPending.proposedPrice) * parseFloat(exchangeRate.usdToGuarani)).toLocaleString('es-PY')}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                {showCounterForm && (
                  <div className="border-t pt-4 space-y-3">
                    ...
                    {counterPrice && exchangeRate && (
                      <p className="text-sm text-muted-foreground mt-2">
                        Equivalente: ₲ {Math.round(parseFloat(counterPrice) * parseFloat(exchangeRate.usdToGuarani)).toLocaleString('es-PY')}
                      </p>
                    )}
                    <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="text-lg font-semibold">
                        ${parseFloat(negotiation.originalPrice).toLocaleString()}
                      </div>
                      <p className="text-sm text-muted-foreground line-through">
                        ₲ {exchangeRate ? Math.round(parseFloat(negotiation.originalPrice) * parseFloat(exchangeRate.usdToGuarani)).toLocaleString('es-PY') : 'Cargando...'}
                      </p>
                      <ArrowRightLeft className="h-4 w-4 text-muted-foreground" />
                      <div className="text-lg font-semibold text-primary">
                        ${parseFloat(negotiation.proposedPrice).toLocaleString()}
                      </div>
                      {negotiation.proposedPrice && exchangeRate && (
                        <p className="text-sm text-muted-foreground mt-2">
                          Equivalente: ₲ {Math.round(parseFloat(negotiation.proposedPrice) * parseFloat(exchangeRate.usdToGuarani)).toLocaleString('es-PY')}
                        </p>
                      )}
